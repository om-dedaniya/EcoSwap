...
const AdminEventManagement = () => {
  ...
  const fetchEventRegistrations = async (eventId) => {
    ...
    try {
      const response = await axios.get(
        `https://ecoswap-e24p.onrender.com/api/event-registrations/${eventId}`
      );
  ...
  const sendMessageToParticipants = async () => {
    if (!selectedEvent || !message.trim()) {
      Swal.fire(
        "Warning",
        "Please select an event and enter a message.",
        "warning"
      );
      return;
    }
    ...
    try {
      await axios.post(
        "https://ecoswap-e24p.onrender.com/api/send-event-message",
        {
      ...
      Swal.fire(
        "Success",
        "Message sent to all registered members!",
        "success"
      );
      setMessage("");
    } catch (error) {
                ...
                <ul className="space-y-2">
                  {selectedEvent.participants.map((participant, index) => (
                    <li key={participant._id} className="text-gray-700">
                      <strong>{index + 1}.</strong> {participant.memberName} -{" "}
                      {participant.email}
                    </li>
                  ))}
                </ul>
