...
const AdminQuery = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            const res = await axios.get("http://localhost:5000/queries/users", {
    ...
    const fetchMessages = async (userEmail) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await axios.get(`http://localhost:5000/queries/${userEmail}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const sendResponse = async () => {
        ...
        try {
            ...
            const res = await axios.put(
                `http://localhost:5000/queries/${selectedUser}`,
                { response },
                { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    return (
        <div className="max-w-6xl mx-auto px-6 py-10 flex flex-col md:flex-row gap-6">
            ...
            <div className="w-full md:w-2/3 bg-white shadow-lg rounded-lg p-6 flex flex-col">
                <h3 className="text-2xl font-semibold text-gray-800 mb-4">💬 Messages</h3>
